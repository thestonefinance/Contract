...
contract STONE is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 165000 ether;
    // Initial distribution for the day 2-5 STONE-WETH LP -> STONE pool
    uint256 public constant INITIAL_STONE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the STONE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("STONE", "STONE") {
        // Mints 1 STONE to contract creator for initial pool setup
    ...
    function _getStonePrice() internal view returns (uint256 _stonePrice) {
        try IOracle(stoneOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("STONE: failed to fetch STONE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _stonePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_stonePrice >= taxTiersTwaps[tierId]) {
    ...
    function setStoneOracle(address _stoneOracle) public onlyOperatorOrTaxOffice {
        require(_stoneOracle != address(0), "oracle address cannot be 0 address");
        stoneOracle = _stoneOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentStonePrice = _getStonePrice();
            currentTaxRate = _updateTaxRate(currentStonePrice);
            if (currentStonePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_stonePool != address(0), "!_stonePool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_stonePool, INITIAL_STONE_POOL_DISTRIBUTION);
